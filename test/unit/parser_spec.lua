...
describe("Parser tests", function ()
    it("Parse a basic snippet", function ()
        ...
        assert.is_same({type = 'tabstop', id = 2, children = {}},
            result[#result])
    end)
    ...
    it("Parse a choice stop", function ()
        ...
        assert.is_same(
            {type = 'choice', id = 2, children = {[1] = 'option1'},
                choices = {'option1', 'option2', 'option3'}}, result[#result])
    end)
    it("Parse an empty placeholder", function ()
        ...
        assert.is_true(ok)
        assert.is_same(pos, #snip + 1)
        assert.is_same({type = 'placeholder', id = 1, children = {}},
            result[1])
    end)
    ...
    it("Parse variables", function ()
        ...
        assert.is_true(ok)
        assert.is_same(pos, #snip + 1)
        assert.is_same({type = 'variable', name = 'TM_CURRENT_YEAR', children = {}},
            result[#result])
    end)
    ...
    it("Parse single ending character", function ()
        ...
        assert.is_true(ok)
        assert.is_same(pos, #snip + 1)
        assert.is_same({type = 'text', raw = '"', escaped = '"'},
            result[#result])
    end)
    it("Parse SnipMate eval", function ()
        ...
        assert.is_true(ok)
        assert.is_same(pos, #snip + 1)
        assert.is_same({type = 'eval', children =
